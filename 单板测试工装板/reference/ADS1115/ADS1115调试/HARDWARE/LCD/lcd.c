...
{ 
	//*(volatile u16 *)(Bank1_LCD_CMD_ADDR)=regval;//写入要写的寄存器序号	 
	*(volatile u16 *)(Bank1_LCD_CMD_ADDR) = regval ;
}
//写LCD数据
//data:要写入的值
void LCD_WR_DATA(u16 data)
{										    	   
	//*(volatile u16 *)(Bank1_LCD_DATA_ADDR)=data;	
		*(volatile u16 *)(Bank1_LCD_DATA_ADDR) = data;
}
//读LCD数据
//返回值:读到的值
...
{										    	   
  ...
  return (*(volatile u16 *)(Bank1_LCD_DATA_ADDR));	
}					   
//写寄存器
//LCD_Reg:寄存器地址
//LCD_RegValue:要写入的数据
...
{	
	//*(volatile u16 *)(Bank1_LCD_CMD_ADDR) = LCD_Reg;		//写入要写的寄存器序号	 
	//*(volatile u16 *)(Bank1_LCD_DATA_ADDR) = LCD_RegValue;//写入数据	 
	
	*(volatile u16 *)(Bank1_LCD_CMD_ADDR) = LCD_Reg;
	*(volatile u16 *)(Bank1_LCD_DATA_ADDR)= LCD_RegValue;
}	   
//读寄存器
//LCD_Reg:寄存器地址
//返回值:读到的数据
...
{
 	*(volatile u16*)(Bank1_LCD_CMD_ADDR)=lcddev.wramcmd;	  
}	 
//LCD写GRAM
//RGB_Code:颜色值
void LCD_WriteRAM(u16 RGB_Code)
{							    
	*(volatile u16 *)(Bank1_LCD_DATA_ADDR) = RGB_Code;//写十六位GRAM
}
...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(*(volatile u16 *)(Bank1_LCD_DATA_ADDR))r=0;							//dummy Read	   
	opt_delay(2);	  
 	r=*(volatile u16 *)(Bank1_LCD_DATA_ADDR);  		  						//实际坐标颜色
 	if(lcddev.id==0X9341)//9341要分2次读出
 	{
		opt_delay(2);	  
		b=*(volatile u16 *)(Bank1_LCD_DATA_ADDR); 
		g=r&0XFF;//对于9341,第一次读取的是RG的值,R在前,G在后,各占8位
		g<<=8;
	}else if(lcddev.id==0X6804)r=*(volatile u16 *)(Bank1_LCD_DATA_ADDR);//6804第二次读取的才是真实值
...
void LCD_DrawPoint(u16 x,u16 y)
{
	...
	*(volatile u16 *)(Bank1_LCD_DATA_ADDR)=POINT_COLOR; 
}
//快速画点
//x,y:坐标
//color:颜色
...
//*===========GPIO For the LCD_Bus========================*/	
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_FSMC,ENABLE);	//使能FSMC时钟
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE,ENABLE);//使能PORTB,D,E,G以及AFIO复用功能时钟
...
{  
 	...
 	for(i=0;i<height;i++)
	{
		...
		LCD_WriteRAM_Prepare();     //开始写入GRAM
		for(j=0;j<width;j++)*(volatile u16 *)(Bank1_LCD_DATA_ADDR)=color[i*height+j];//写入数据 
	}	  
}  
//画线
//x1,y1:起点坐标
