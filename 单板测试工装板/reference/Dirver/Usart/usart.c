...
void Usart1_Module_Init(unsigned long Baud){
    ...
    GPIO_Init(GPIOC,GPIO_Pin_2,GPIO_Mode_In_FL_IT);  //设置PC2端口浮空输入
    GPIO_Init(GPIOC,GPIO_Pin_3,GPIO_Mode_Out_OD_HiZ_Fast);//设置PC3端口为推挽输出
    
    USART_DeInit(USART1); //初始化端口为默认值
  
    USART_Init(USART1,(uint32_t)Baud ,USART_WordLength_8b ,USART_StopBits_1 ,USART_Parity_No ,USART_Mode_Rx | USART_Mode_Tx);
...
void Usart3_Module_Init(unsigned long Baud)
{
    ...
    GPIO_Init(GPIOG,GPIO_Pin_0,GPIO_Mode_In_FL_IT);  //设置PG0端口为上拉输入
    GPIO_Init(GPIOG,GPIO_Pin_1,GPIO_Mode_Out_OD_HiZ_Slow);//设置PG1端口为推挽输出
    
    USART_DeInit(USART3); //初始化端口为默认值
  
    USART_Init(USART3,(uint32_t)Baud ,USART_WordLength_8b ,USART_StopBits_1 ,USART_Parity_No ,USART_Mode_Rx | USART_Mode_Tx );
