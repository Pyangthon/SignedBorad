...
/*加密*/
void AesEncrypt(unsigned char *SreData,unsigned char Len, unsigned char *AesKey)
{
    unsigned char i;
    unsigned char chainCipherBlock[16];
    for(i=0;i<16;i++) 
      AES_Key_Table[i]=AesKey[i];//做运算之前先要设置好密钥，这里只是设置密钥的DEMO。
    ...
    for(i = 0; i < Len ;i = i+16)
    {
        ...
        aesEncrypt(&SreData[i], chainCipherBlock);//AES加密，数组dat里面的新内容就是加密后的数据。
    }    
}
...
/*解密*/
void AesDecrypt(unsigned char *SreData,unsigned char Len, unsigned char *AesKey)
{
    unsigned char i;
    unsigned char chainCipherBlock[16];
    for(i=0;i<16;i++) 
    ...
    for(i = 0; i < Len ;i = i+16)
    {
        memset(chainCipherBlock,0x00,sizeof(chainCipherBlock));
        aesDecInit();//在执行解密初始化之前可以为AES_Key_Table赋值有效的密码数据
        aesDecrypt(&SreData[i], chainCipherBlock);//AES解密，密文数据存放在dat里面，经解密就能得到之前的明文。       
    }    
}
...
void AES_Test(void)
{
  unsigned char dat[32]={0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,
                               0x09,0x0A,0x0B,0x0C,0x0D,0x0E,0x0F,0x00};
	unsigned char chainCipherBlock[16];
    unsigned char AesKey[16] = {0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x08,
                      0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
    unsigned char i;
	for(i=0;i<16;i++) 
        AES_Key_Table[i]=AesKey[i];//做运算之前先要设置好密钥，这里只是设置密钥的DEMO。

    AesEncrypt(dat,32,AesKey);
    AesDecrypt(dat,32,AesKey);
