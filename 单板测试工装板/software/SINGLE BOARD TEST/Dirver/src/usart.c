...
//    memset(&g_uart_rxbuf, 0, sizeof(g_uart_rxbuf));
    ...
    GPIO_Init(GPIOC,GPIO_Pin_3,GPIO_Mode_Out_PP_Low_Fast);//设置PC3端口为推挽输出
...
void Usart2_Module_Init(unsigned long Baud)
{
    ...
    // 设置串口 字长为8位，停止位1位，无奇偶校验，发送和接收双向
...
{ 
    USART_SendData8(USART3,(u8)data);  //发送8位数据
    while(!USART_GetFlagStatus (USART3,USART_FLAG_TXE));//等待发送完毕
    return data; 
}
